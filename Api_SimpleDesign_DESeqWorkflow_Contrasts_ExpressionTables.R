...
Mum_dir='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/'
...
w1='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/F_CremTG_vs_F_CTR'
w2='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/F_HDAC2KO_vs_F_CTR'
w3='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/F_TGKO_vs_F_CremTG'
w4='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/F_TGKO_vs_F_CTR'
w5='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/M_CremTG_vs_M_CTR'
w6='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/M_HDAC2KO_vs_M_CTR'
w7='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/M_TGKO_vs_M_CremTG'
w8='W:/01_GenEpi_Projects/Schulte_2025/1.MalesOrFemalesONLY/M_TGKO_vs_M_CTR'

#wdir list
wdir_list<- list(w1, w2, w3, w4, w5, w6, w7, w8)

wdir_lenght <- length(wdir_list)

#for testing
#i=1

for (i in 1:wdir_lenght){

#-----------Define Variables
##Working directory
  #wdir='W:/Clara/NGS_274_825'
  wdir= wdir_list[[i]]
...
############# Differential Expression Analysis - contrasts, volcano plots ... ##################
  ...
  Comparisons <- list(c("Group","CremTG","CTR"),
                      c("Group","HDAC2_KO","CTR"),
  ...
  #for (i in 1:number_comparisons) 
    ...
    sink(paste0("Summary_DESeq2_Results_",comparison_lab,".txt"))
...
####FOR HEATMAP: Create a subset dataset with only genes with padj <pval_thr sorted by decreasing |Shrunk_LFC|
    ...
    tiff(paste0("plotMA.tiff"),units = 'in', width = 5, height = 5, res=300)
    print(plotMA)
    dev.off()
    
    plotMA_shrunk <-plotMA(resLFCSh,ylim=c(-2,2))
    tiff(paste0("plotMA_shrunk.tiff"),units = 'in', width = 5, height = 5, res=300)
    print(plotMA_shrunk)
    dev.off()
  ...
  ####Prepare table for saving results
    ...
    write.table(res_df,paste0("DESeq2_Results_",comparison_lab,".txt"),sep="/t", quote=F,row.names = FALSE)
    write.xlsx(res_df,paste0("DESeq2_Results_",comparison_lab,".xlsx"))
    write.xlsx(res_df_sig_shrLFC,paste0("DESeq2_Results_padj",pval_thr,comparison_lab,".xlsx"))
...
########  Volcano plot
    ...
    #pdf(file = paste('VolcanoPlot_',comparison, '.pdf', sep=''), width = 8, height = 12) # you can change the size of the output file
    tiff(paste0('VolcanoPlot_',comparison, '.tiff', sep=''),units = 'in', width = 8, height = 12, res=300)
...
########  Heatmap with vst data - Top50 genes by padj
    ...
    #Save plot
    tiff(paste0("Heatmap_TOP50genes_bypadj_vstdata.tiff"),units = 'in', width = 5, height = 5, res=300)
...
########  Heatmap with vst data - Genese sorted by shrunkFoldChange values
    ...
    #Save plot
    tiff(paste0("Heatmap_TOP50genes_byshrLFC_vstdata.tiff"),units = 'in', width = 5, height = 5, res=300)
